...
def match_images_by_id(
    ...
    Trả về: (mapping id->bytes, orig_names id->filename, matched_ids, unmatched_ids)
    """
    # Index ảnh theo "stem" không dấu
...
def build_config_from_sidebar() -> Tuple[ThumbnailConfig, dict]:
    ...
    with st.sidebar:
        ...
        with st.expander("📐 Layout sản phẩm", expanded=True):
            top_m = st.slider(
                "Mép trên → SP (px)",
                0, 150, DEFAULT_TOP_MARGIN,
                help="Khoảng cách từ mép trên thumbnail đến đỉnh sản phẩm. Mặc định 36.",
            )
            bot_m = st.slider(
                "Mép dưới → SP (px)",
                0, 250, DEFAULT_BOTTOM_MARGIN,
                help="Khoảng cách từ mép dưới thumbnail đến đáy sản phẩm. Mặc định 148.",
            )
            side_p = st.slider("Padding 2 bên (px)", 0, 80, 20)
        ...
        with st.expander("🪄 Tách nền (tuỳ chọn)", expanded=False):
            bg_mode_label = st.radio(
                "Chế độ tách nền",
                ["Không tách", "Tách nền trắng (nhanh)", "Tách bằng AI (rembg)"],
                index=0,
                help="Tách nền trắng dùng flood-fill — không cần AI, giữ chi tiết tốt. "
                     "Tách AI dùng mô hình U2Net — chính xác cho ảnh phức tạp nhưng lần đầu cần tải model.",
            )
            bg_mode = {"Không tách": "none", "Tách nền trắng (nhanh)": "white",
                       "Tách bằng AI (rembg)": "ai"}[bg_mode_label]
    ...
    <div style='display:flex; align-items:center; justify-content:space-between;
      ...
      <div>
        ...
        <div style='opacity:0.9; font-size:14px;'>
          Tạo thumbnail sản phẩm 600×600 hàng loạt · Montserrat · Smart-fit · Auto-shrink text
        </div>
      </div>
      <div style='text-align:right; font-size:12px; opacity:0.85;'>
        <div>Rule mặc định: top 36px · bot 148px</div>
        <div>Font size: 20.4 · Auto co giãn</div>
      </div>
    </div>
    """,
    unsafe_allow_html=True,
...
with tab_upload:
    ...
    if df is not None and imgs:
        ...
        with c1:
            st.markdown(f"<div class='stat-card'><h3>{len(df)}</h3><p>Dòng trong Excel</p></div>", unsafe_allow_html=True)
        with c2:
            st.markdown(f"<div class='stat-card'><h3>{len(matched)}</h3><p>Khớp ảnh</p></div>", unsafe_allow_html=True)
        with c3:
            st.markdown(f"<div class='stat-card'><h3>{len(unmatched)}</h3><p>Thiếu ảnh</p></div>", unsafe_allow_html=True)
...
with tab_export:
    ...
    if df is None or not mapping:
        ...
        if st.button("🚀 Tạo ZIP", type="primary", use_container_width=True):
            bg = load_background()
            fmt = out_meta["format"].lower()
            mime_img = "image/png" if fmt == "png" else "image/jpeg"
