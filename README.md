...
- 📐 **Smart-fit ảnh** về 600×600 — không méo, không mất chi tiết, không vỡ góc, dù ảnh gốc to hay nhỏ
- ✒️ **Text auto co giãn** — text ngắn giữ nguyên 20.4px (Montserrat), text dài tự shrink vừa ô pill
- 🎨 **Pill shadow đẹp** — vẽ trực tiếp bằng code (không phụ thuộc asset), 2 pill ở góc trên trái giống mẫu
- 🪄 **Tách nền** tuỳ chọn: flood-fill nền trắng (nhanh, không cần AI) hoặc rembg/U2Net (AI, chính xác)
- 👁️ **Preview grid** live — mọi chỉnh sửa ở sidebar đều thấy ngay
- 📦 **Xuất ZIP** chứa tất cả thumbnail + file `cms.csv` + `cms.xlsx` để nạp vào CMS
- 🔐 **Mật khẩu** qua `st.secrets` — source public cũng không lộ mật khẩu
- 🎨 **UI tối ưu**: dark-hidden source, theme tím gradient, emoji, thống kê tức thì
...
# Mở file và sửa APP_PASSWORD = "mat-khau-cua-ban"
...
5. Click **"Advanced settings"** → tab **Secrets**, dán vào:
   ```toml
