...
class ThumbnailConfig:
    ...
    # Layout 2 pill (giống mẫu, có thể chỉnh qua UI)
    pill_left: int = PILL_LEFT
    pill_right: int = PILL_RIGHT
    pill_height: int = PILL_HEIGHT
    pill1_top: int = PILL1_TOP
    pill2_gap: int = 11                 # khoảng cách giữa 2 pill
    # Shadow
    shadow_offset_x: int = 2
    shadow_offset_y: int = 3
    shadow_blur: int = 6
    shadow_opacity: int = 110
    # Chế độ hiển thị 1 pill (khi chỉ có 1 text)
    single_pill_mode: str = "auto"      # "auto": tự ẩn pill rỗng
...
def build_thumbnail(
    ...
    fitted = smart_fit(prod, area_w, area_h)
    px = (W - fitted.width) // 2
